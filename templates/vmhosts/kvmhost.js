...
$(function () {
  var table = $("#clientslist").DataTable({
    "serverSide": true,
    "ajax": "{% url 'vmskvmhostdefineddata' %}",
    "language": {
      "emptyTable": "No KVM Hosts defined"
    },
    "bAutoWidth": false,
    "columns": [
...
{% if perms.clients.change_clients %}
          ret += btn + 'onclick="location.href=\'{% url 'clientsedit_rel' %}'+data[0]+'/?b={% url 'vmskvmhostdefined' %}\';"><i class="fa fa-wrench" data-toggle="tooltip" data-original-title="Edit"></i></button>\n';
{% endif %}
...
{% include 'widgets/helpbutton.js' with helppage='vmhosts.kvmhost' %}