...
$(function () {
  var table = $("#clientslist").DataTable({
    "serverSide": true,
    "ajax": "{% url 'vmsproxmoxdefineddata' %}",
    "language": {
      "emptyTable": "No Proxmox Hosts defined"
    },
    "bAutoWidth": false,
    "columns": [
...
{% if perms.clients.change_clients %}
          ret += btn + 'onclick="location.href=\'{% url 'clientsedit_rel' %}'+data[0]+'/?b={% url 'vmsproxmoxdefined' %}\';"><i class="fa fa-wrench" data-toggle="tooltip" data-original-title="Edit"></i></button>\n';
{% endif %}
...
{% include "widgets/refreshbutton.js" %}
  ...
  var tableguests = $("#guestslist").DataTable({
    "data": [],
    "language": {
      "emptyTable": "No Proxmox Guests available"
    },
    "bAutoWidth": false,
    "columns": [
      { "sClass": "vertical-align",
  ...
  function loadguests(name){
    tableguests.context[0].oLanguage.sEmptyTable = "No Proxmox Guests available";
    tableguests.clear();
    $('#proxmoxguestlist').removeClass('box-danger').addClass('box-primary');
    $('#vmguestsloading').show();
    $('#guestsrefresh').prop("disabled", true);
    ajaxguests = $.ajax({
        url: "{% url 'vmsproxmoxvmlist_ref' %}" + name + '/',
        type: "GET",
        dataType: "json",
        success: function(data){
            tableguests.rows.add(data['data']);
            if (data['err'] != null){
                $('#proxmoxguestlist').removeClass('box-primary').addClass('box-danger');
                switch (data['err']){
                case 1:
                    tableguests.context[0].oLanguage.sEmptyTable = "Cannot connect to Proxmox Host."
                    break;
                case 2:
                    tableguests.context[0].oLanguage.sEmptyTable = "No Proxmox Plugin installed on Host."
                    break;
                }
            }
            tableguests.draw();
...
{% include 'widgets/helpbutton.js' with helppage='vmhosts.proxmox' %}